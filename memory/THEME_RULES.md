...
- Loader entry, each content pop-in, the final ready text, and the complete background exit use shared non-positional Roblox asset sounds through the central audio registry. The bar-collapse pop-out sound is forbidden. Exit uses the soft two-second UI transition asset `90657541635248` (`sfx_ui_whoosh_medsmall`) at playback speed 1 and volume 0.08; it begins at the same instant as the downward exit. The previous reverse horror-like whoosh is forbidden. Missing or permission-restricted audio must never stop an animation.
