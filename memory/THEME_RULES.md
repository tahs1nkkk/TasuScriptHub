...
- The 280×280 reusable `TasuHub` icon occupies the middle area. Its primary source is the intentionally multicolor Roblox image asset `138667112902223`; the prior theme-colored vector mark remains its layout-stable fallback until the image loads or whenever it is unavailable. After its staged reveal, the icon stays fixed on its center anchor and runs a slow 6.4 second sine loop: rotation rocks between −6° and +6° while scale breathes continuously from 1 to 1.08. The 480×28 progress bar and 18 px status are positioned at 80% viewport height; the bottom-right version label retains its safe margin.
