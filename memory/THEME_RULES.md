...
- The 280×280 reusable `TasuHub` icon occupies the middle area. Its primary source is the intentionally multicolor decal `138667112902223`. Because direct decal delivery requires authorization and `rbxthumb` did not resolve inside the executor UI, runtime code queries the Roblox thumbnail API, downloads the returned PNG, and registers it through the executor's custom-asset API. The prior theme-colored vector mark remains its layout-stable fallback until that path succeeds or whenever the required executor capabilities are unavailable. After its staged reveal, the icon stays fixed on its center anchor and runs a slow 6.4 second sine loop: rotation rocks between −6° and +6° while scale breathes continuously from 1 to 1.08. The 480×28 progress bar and 18 px status are positioned at 80% viewport height; the bottom-right version label retains its safe margin.
...
- At 100%, the progress bar shrinks toward its center and disappears silently over 0.7 seconds. The status then changes to green `Herşey Hazır!`, shifts upward from 80% to 76% viewport height, scales to 1.40 over 1.2 seconds, and plays the final completion pop. This completed state remains for 1.8 seconds before exit. No other content has an independent exit: the complete loader accelerates downward and fades linearly over 2 seconds until it leaves the viewport; only then is it hidden and destroyed.
- Loader entry, each content pop-in, the final ready text, and the complete background exit use shared non-positional Roblox asset sounds through the central audio registry. The bar-collapse pop-out sound is forbidden. Exit uses reverse-whoosh Roblox asset `9120699503`; its seven-second source runs at `7/3` speed for an exact three-second cue. The reverse cue begins at the same instant as the downward exit, 0.3 seconds earlier than the preceding timing revision. Missing or permission-restricted audio must never stop an animation.
...
- General icons live in the shared icon library and accept size/color/Z-index options. `TasuHub` is the first general icon and must be reused rather than redrawn in each location. Its approved multicolor asset may preserve original colors; its vector fallback inherits the theme color.
