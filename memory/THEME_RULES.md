...
- Loader entry, each content pop-in, the final ready text, and the complete background exit use shared non-positional Roblox asset sounds through the central audio registry. The bar-collapse pop-out sound is forbidden. Exit uses reverse-whoosh Roblox asset `9120699503`; its seven-second source runs at `7/3` speed for an exact three-second cue. The reverse cue begins exactly two seconds before the downward exit starts, during the final bar-collapse/ready sequence, so its rise is already audible when the screen begins moving. Missing or permission-restricted audio must never stop an animation.
