...
Implemented rework step: the old card/radial loader was replaced with the approved translucent dark loader. `loader.lua` now owns the reusable vector icon registry entry `TasuHub`, tracked background blur, dense lower-60% responsive dot grid, a shadow-free loader composition, semantic loader tokens, a white track with gray horizontal progress fill, clipped negative percentage text, live runtime-bound Turkish status text, Roblox-linked audio cues, and the whole-background downward exit.
