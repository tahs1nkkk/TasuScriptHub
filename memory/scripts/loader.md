...
Implemented rework step: the old card/radial loader was replaced with the approved translucent dark loader. `loader.lua` now owns the reusable 280 px vector icon registry entry `TasuHub`, its slow breathing/left-right ambient loop, tracked background blur, a constant-speed upward 60×22 halftone loop driven through 22 optimized row groups, remembered dot baselines, a shadow-free loader composition, semantic loader tokens, a white track with gray horizontal progress fill, clipped negative percentage text, live runtime-bound Turkish status text, Roblox-linked audio cues, and the whole-background downward/fade exit.
