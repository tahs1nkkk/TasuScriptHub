...
- Loader branded-asset revision: `TasuHub` now resolves asset `138667112902223` through the shared icon registry. The old frame geometry is intentionally retained only as the live load-failure fallback; it is not dead duplicate artwork. Exit-audio lead is reduced to 0.4 seconds through the existing token with no new trigger path.
