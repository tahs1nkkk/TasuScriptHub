...
- Loader branded-asset revision: `TasuHub` now resolves asset `138667112902223` through the shared icon registry's accessible `rbxthumb` URI; the authorization-blocked direct `rbxassetid` path is removed. The old frame geometry is intentionally retained only as the live load-failure fallback; it is not dead duplicate artwork. The exit-audio token now represents a 0.3-second post-exit-start delay, with no duplicate trigger path.
