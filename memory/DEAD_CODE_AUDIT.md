...
- Corner-action assignment audit: both semantic definitions and asset IDs are consumed during construction; slot 1's callback reaches the single remote catalog controller, while slot 2's nil action is an intentional next-step boundary. No duplicate catalog/general-menu launcher or unload implementation exists.
...
- Corner dock revision: the obsolete 78/84/74 geometry, 38 px static icon, 3 px stroke, fixed bottom-right anchor, 30×44 arrow/glyph/rotation, arrow sound path, and one-direction hide offsets are removed. The 70/76/67 button sizes, synchronized 48/52/46 icon sizes, 6 px stroke, padded clipping-safe stack bounds, four directional hide paths, nearest-edge drag state, resize re-docking, and click suppression remain live; its interim 6×64 edge line is superseded by the later visibility-bar revision. Both audio presets remain live: `ButtonHover` only on entry and `ButtonClick` only on valid activation.
- Loader checkpoint/icon revision: the 280 px icon, generic 0.82 → 1.035 icon reveal, direct arbitrary-progress tween, 1.4-second `MotionProgress` token, and fixed `UI.PlaySound` preset-only signature are removed. Live replacements are the 560 px icon, dedicated 0.58 → 1.16 reveal, a monotonic 30-checkpoint worker, target waits, per-step rising playback-speed overrides, and short checkpoint-sound cleanup. Every new function/token has a construction, worker, final-wait, or cleanup consumer.
- Loader asset-gate/feedback revision: the live 560 px icon, one-shot empty-canvas failure path, 6.4-second motion loop, 40% grid start, 0.045 checkpoint volume, 0.72 starting pitch, generic ready font, and shared-speed ready pop are superseded. Their only live replacements are the 392 px icon, bounded 20-attempt prerequisite gate with invisible failure cleanup, `6.4/1.5` motion loop, 44% grid start, volume-0.09 checkpoint tick beginning at pitch 0.50, Nunito Black ready state, and speed-0.60 final pop. `LoaderCheckpointInterval` is removed in favor of the semantically accurate `LoaderCheckpointMinInterval`; every new token has a runtime consumer and no Studio-only or fallback path was introduced.
- Loader completion-scale closure: the 1.40 ready-state scale is superseded by the sole `LoaderReadyScale = 2.10` token, which is consumed by the existing completion tween. No duplicate ready-text size path or one-off `TextSize` mutation was introduced; the loader system is now user-approved and closed.
- Corner visibility-bar revision: the borderless `Signal` 6×64 `VisibilityLine`, 0.30 resting transparency, and directionless state are removed. The live replacement is one 18×64/64×18 `Layer` `VisibilityBar` with a 2 px `Base` outline, a 16 px upward vertical offset, and one child `Signal` arrow whose rotation is derived from the existing dock side and hidden state. Arrow updates reuse `applyCornerDock` and `SetCornerButtonsHidden`; no parallel toggle state, input connection, sound path, or private palette was introduced.
- Game-catalog revision 1: slot 1's temporary unload callback and "catalog view not implemented" boundary are removed. Live replacements are the versioned remote module loader, one `GameCatalogRoot`, one controller, structured execution result/duplicate-lock service, automatic cover resolver, three-column card grid, local card filter, and anchor-aware transition path. The old hidden catalog builder remains compatibility-only and now delegates execution to `UI.ExecuteCatalogEntry`; no old catalog frame, modal, toast, palette, or banner card is reused by the visible module. All new module functions, tokens, callbacks, connections, and exports have construction, interaction, or cleanup consumers.
- Game-catalog revision 2: the 1000×680 geometry, 0.80 body transparency, transition fill, animated shell `Size`, stretch/expand/content/contract/return phases, static footer copy/count, external status divider, run hint, and click-toggled search are removed. The live replacements are fixed 780×520 geometry, 0.20 body transparency, whole-window 0↔1 scale/origin transition, header/footer drag path, hover/pinned search with adjacent count, remote preview metadata, cover-overlay status strip, feature summary, semantic feedback outline, card shake, and revision-guarded outcome toast. Old catalog motion tokens have zero references; every new color/layout/motion token is consumed by the remote-module context. Static Lua parsing passes for `loader.lua`, `game_catalog.lua`, and `mm2.lua`; no Studio-only path or legacy presentation was enabled.
