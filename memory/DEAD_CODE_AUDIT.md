...
- Loader background revision: removed `LoaderTopFade`, its 1000 px exit offset, and all gradient keypoints. Background depth now comes from tracked blur, a lower viewport vector dot grid, and shared single-copy shadows.
- Loader shadow revision: removed the four-icon, three-bar, and three-status shadow arrays. Each loader element now owns one black, right/down-shifted soft shadow copy.
