...
- Loader branded-asset revision: `TasuHub` now resolves decal `138667112902223` through a one-shot thumbnail-API/download/custom-asset pipeline; both the authorization-blocked direct `rbxassetid` path and the executor-incompatible direct `rbxthumb` display path are removed. The old frame geometry is intentionally retained only as the live capability/load-failure fallback; it is not dead duplicate artwork. The exit-audio delay token is zero, synchronizing its sole trigger with exit motion.
