...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses reverse-whoosh asset `9120699503`, whose seven-second source is normalized to exactly three seconds with playback speed `7/3`. The exit cue is scheduled from the bar-collapse phase to begin at the same instant as the downward loader movement, 0.3 seconds earlier than the preceding revision. Content pop-in and the final ready-text cue use `140323850218372`. The bar-collapse sound and its `PopOut` preset are removed. Audio failure is non-blocking.
...
`UI.IconLibrary.TasuHub` reads decal ID and cache path from shared registry entry `UI.IconAssets.TasuHub`. An asynchronous worker queries `thumbnails.roblox.com`, downloads the current 420×420 PNG, saves `TasuHub/Icons/TasuHub.png`, resolves it with `getcustomasset`/`getsynasset`, and then atomically replaces the scalable frame-built fallback. The worker checks instance ownership before applying a late response. `UI.CreateIcon(parent, "TasuHub", options)` accepts `Size`, `Color`, and `ZIndex`; `Color` applies to the fallback while the approved multicolor PNG preserves its original artwork.
...
Run the canonical executor entrypoint on a fresh client and on a second execution. Confirm the game scene is visibly blurred behind the 20%-transparent dark scrim; no top fade or shadow object remains; and the denser responsive dot grid begins around 40% viewport height. Verify every row travels upward at the same constant speed, reaches zero opacity at both travel endpoints without a wrap flash, and preserves all 60 columns. On a full-capability executor, confirm decal `138667112902223` replaces the vector fallback without layout shift, then breathes and rocks around its fixed center anchor without translating. Confirm the fallback remains visible if HTTP, file, thumbnail, or custom-asset loading is blocked. Confirm the percentage remains white over both track states and real construction headings visibly advance without a fake timer. At 100%, confirm bar collapse is silent, the only completion pop occurs on the larger green `Herşey Hazır!`, and the ready state remains for 1.8 seconds. Confirm the three-second reverse whoosh starts simultaneously with the downward movement. During the 2 second exit, confirm every visual fades at one linear rate while the complete loader accelerates downward and dot/icon loops stop only after the loader clears the screen. Confirm no animation connection, blur, overlay, legacy UI, or duplicate loader remains after exit/reload. Audio permission failures may silence a cue but may not delay or break the sequence. Review the executor console for property/tween errors.
