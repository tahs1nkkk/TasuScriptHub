...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses reverse-whoosh asset `9120699503`, whose seven-second source is normalized to exactly three seconds with playback speed `7/3`. Content pop-in and the final ready-text cue use `140323850218372`. The bar-collapse sound and its `PopOut` preset are removed. Audio failure is non-blocking.
