...
- The background, blur, and bottom grid enter over 0.5 seconds. During exit the complete loader moves downward over 1.7 seconds with Quint-In acceleration, its full `CanvasGroup` tree fades uniformly with a 1.7 second linear tween, and blur animates back to zero.
...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses soft, modern UI-whoosh asset `90657541635248` at volume 0.07. Playback speed `2/3` extends its native two-second recording to three seconds; `PitchShiftSoundEffect.Octave = 1.5` compensates pitch so stretching does not turn it dark or horror-like. The cue begins simultaneously with the 1.7-second downward movement and finishes as a soft tail after the screen clears. Content pop-in and the final ready-text cue use `140323850218372`. The reverse horror-like whoosh, bar-collapse sound, and `PopOut` preset are removed. Audio failure is non-blocking.
