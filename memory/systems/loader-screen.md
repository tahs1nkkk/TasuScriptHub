...
- The 280×280 reusable `TasuHub` mark sits around 42% viewport height. Roblox asset `138667112902223` renders at original multicolor through the `rbxthumb://type=Asset` resolver; direct `rbxassetid://` delivery is not used because the asset requires authorization. The former vector mark remains hidden underneath as an automatic fallback until the thumbnail loads or if it is unavailable. Once its reveal settles, the icon stays position-locked on its center anchor, breathes from scale 1 to 1.08, and rocks from −6° to +6° on a smooth 6.4 second sine loop. The 480×28 fully rounded track sits around 80% viewport height, with an 18 px live status line directly below it. `TasuHub · v<version>` uses 13 px text and is right-aligned in the bottom-right corner.
...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses reverse-whoosh asset `9120699503`, whose seven-second source is normalized to exactly three seconds with playback speed `7/3`. The exit cue is scheduled from the bar-collapse phase to begin 0.3 seconds after the downward loader movement starts, 0.7 seconds later than the preceding revision. Content pop-in and the final ready-text cue use `140323850218372`. The bar-collapse sound and its `PopOut` preset are removed. Audio failure is non-blocking.
...
`UI.IconLibrary.TasuHub` resolves shared registry entry `UI.IconAssets.TasuHub` to `rbxthumb://type=Asset&id=138667112902223&w=420&h=420`. It retains the scalable frame-built mark as a fallback and switches atomically when `ImageLabel.IsLoaded` becomes true. `UI.CreateIcon(parent, "TasuHub", options)` accepts `Size`, `Color`, and `ZIndex`; `Color` applies to the fallback while the approved multicolor thumbnail preserves its original artwork.
...
Run the canonical executor entrypoint on a fresh client and on a second execution. Confirm the game scene is visibly blurred behind the 20%-transparent dark scrim; no top fade or shadow object remains; and the denser responsive dot grid begins around 40% viewport height. Verify every row travels upward at the same constant speed, reaches zero opacity at both travel endpoints without a wrap flash, and preserves all 60 columns. Confirm the `rbxthumb` source for asset `138667112902223` replaces the vector fallback without layout shift, then breathes and rocks around its fixed center anchor without translating. Confirm the fallback remains visible if thumbnail loading is blocked. Confirm the percentage remains white over both track states and real construction headings visibly advance without a fake timer. At 100%, confirm bar collapse is silent, the only completion pop occurs on the larger green `Herşey Hazır!`, and the ready state remains for 1.8 seconds. Confirm the three-second reverse whoosh starts 0.3 seconds after the downward movement. During the 2 second exit, confirm every visual fades at one linear rate while the complete loader accelerates downward and dot/icon loops stop only after the loader clears the screen. Confirm no animation connection, blur, overlay, legacy UI, or duplicate loader remains after exit/reload. Audio permission failures may silence a cue but may not delay or break the sequence. Review the executor console for property/tween errors.
