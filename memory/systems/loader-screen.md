...
- The 280×280 reusable `TasuHub` mark sits around 42% viewport height. Roblox image asset `138667112902223` renders at original multicolor; the former vector mark remains hidden underneath as an automatic fallback until the asset loads or if it is unavailable. Once its reveal settles, the icon stays position-locked on its center anchor, breathes from scale 1 to 1.08, and rocks from −6° to +6° on a smooth 6.4 second sine loop. The 480×28 fully rounded track sits around 80% viewport height, with an 18 px live status line directly below it. `TasuHub · v<version>` uses 13 px text and is right-aligned in the bottom-right corner.
...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses reverse-whoosh asset `9120699503`, whose seven-second source is normalized to exactly three seconds with playback speed `7/3`. The exit cue is scheduled from the bar-collapse phase to begin 0.4 seconds before the downward loader movement, 1.6 seconds later than the preceding revision. Content pop-in and the final ready-text cue use `140323850218372`. The bar-collapse sound and its `PopOut` preset are removed. Audio failure is non-blocking.
...
`UI.IconLibrary.TasuHub` resolves shared registry entry `UI.IconAssets.TasuHub` to Roblox asset `138667112902223`. It retains the scalable frame-built mark as a fallback and switches atomically when `ImageLabel.IsLoaded` becomes true. `UI.CreateIcon(parent, "TasuHub", options)` accepts `Size`, `Color`, and `ZIndex`; `Color` applies to the fallback while the approved multicolor image preserves its original artwork.
