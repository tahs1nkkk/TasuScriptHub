...
- Audio registry: entry uses Roblox asset `1127797047`; exit uses soft, modern UI-whoosh asset `90657541635248` at volume 0.08. Its native two-second duration matches the full downward exit and begins simultaneously with that movement. Content pop-in and the final ready-text cue use `140323850218372`. The reverse horror-like whoosh, bar-collapse sound, and `PopOut` preset are removed. Audio failure is non-blocking.
