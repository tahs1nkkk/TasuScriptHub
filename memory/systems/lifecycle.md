...
Transient loader sounds remain tracked instances. Their cleanup delay respects an explicit target duration plus a one-second margin (minimum four seconds), preventing the two-second soft UI exit cue from being destroyed before playback completes. The exit cue's scheduled start checks loader ownership and visibility before creating sound state, so unload cannot resurrect audio after cleanup.
