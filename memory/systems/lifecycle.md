...
The post-loader corner-action rail is owned by the main tracked `ScreenGui`. Its button hover, leave, press, release, and activation connections plus the arrow's hover/leave/activation connections are registered through the global connection tracker. Hide/show completion uses a revision guard before changing visibility. Late icon workers carry a per-button revision and verify image ownership before assignment; replacing an ID or unloading cannot apply stale image state. Hover sounds are tracked transient instances and share the normal sound cleanup path. During the intermediate shell build, slot 1 invokes the same idempotent `unload` function exported through `env.TasuHub`; this removes the rail, its arrow, connections, late icon ownership, and all other runtime state without constructing an unload screen.
