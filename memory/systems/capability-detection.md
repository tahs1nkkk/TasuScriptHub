...
The TasuHub decal loader requires HTTP, file/folder, and custom-asset capabilities. It resolves the current Roblox thumbnail URL at runtime, writes the PNG into `TasuHub/Icons`, and keeps the vector icon visible if any required capability or request fails. It never substitutes a Studio-only path.
