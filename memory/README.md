...
The loader design is user-approved and closed after the final ready-text scale revision. Preserve it as a finished shared system unless the user explicitly reopens loader work. Game-catalog revision 2 is implemented through the upper corner-action button and remote `game_catalog.lua`: smaller fixed-layout origin transition, draggable header/footer, hover search, preview cards, semantic statuses, and execution feedback. It is not runtime-ready until its executor checklist is completed and the user approves visual revisions. The general menu launched by the lower button remains the following UI phase.
